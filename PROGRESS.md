...
> 当前阅读入口已切换到 `docs/current/START_HERE.md`。本文件保留历史进度，不再作为默认第一阅读材料。

## Current Truth Snapshot - 2026-05-07
...
- Current latest engineering stage: `Stage22B realtime weather dispatch run history and export hardening`.
- Current latest dispatch-side DL result: `Stage20B two-stage neural dispatch policy`.
- Current priority: enter final thesis writing. Freeze the default engineering scope, use the current evidence chain for chapter/table/figure consolidation, and avoid new experiments unless explicitly requested.
...
- Thesis final-writing handoff: `reports/thesis_final_writing_handoff_2026-05-07.md`.
- Current thesis candidates discovered: `thesis/main.docx`, `thesis/main.tex`, `thesis/main.html`, and `reports/胡家铭_本科毕业论文_Final_格式修正版.docx`. Pick one working mother file before content edits to avoid DOCX/TEX drift.
...
| Rawhide weather/market feasibility | Open-Meteo Rawhide 48h forecast fields are available; SPP/WEIS/RTO price data is an extension candidate; configurable price scenarios are required | `reports/stage21a_rawhide_weather_market_feasibility_report.md` |
| Realtime weather dispatch bench | Backend exposes Open-Meteo forecast normalization and a unified dispatch experiment run endpoint; frontend now renders backend-returned weather, dispatch rows, KPI, comparison, sensitivity, and recommendation | `backend/app/weather_experiment.py`, `frontend/src/views/DispatchSimulation.vue` |
| Realtime dispatch run history | Stage22B appends JSONL run records, exposes history/detail/export endpoints, and adds frontend history reload plus JSON/CSV/summary export controls | `data/processed/pvdaq_nsrdb_2020_2022/stage22_weather_experiment_runs.jsonl` |
...
- Stage21A supports "Rawhide weather-driven and configurable price-scenario simulation is feasible", not "Rawhide real settlement revenue is proven".
- Stage22/Stage22B supports "realtime weather API driven dispatch demonstration with reproducible run records", not "measured Rawhide generation" or "real market settlement".
...
- Do not describe Stage21A synthetic price scenarios as local real settlement prices.

### Current Next Task

Recommended next task: `Thesis-Final-1 final manuscript consolidation`.

Stage22B validation already completed in the previous engineering slice: `python -m pytest tests/test_weather_experiment_api.py`, `python -m compileall backend\app\weather_experiment.py backend\app\main.py src\new_energy_sys\api\main.py`, `python scripts\api_smoke_frontend_contract.py`, `npm run lint`, `npm run build`, and browser smoke against temporary ports 8021/3083. Use `reports/thesis_final_writing_handoff_2026-05-07.md`, `reports/project_status_2026-05-05.md`, `reports/module_progress_assessment_2026-05-05.md`, and `docs/reports_index.md` to consolidate thesis chapters, figures, experiment tables, and limitations. Only run new experiments if explicitly requested.

Thesis-Final-1 should first lock the manuscript mother file, then revise by chapter: requirements -> design -> implementation -> testing -> conclusion. Keep these boundaries explicit: LightGBM remains the main PV predictor, Stage20B is policy distillation rather than optimizer replacement, Rawhide is reference simulation rather than measured operation, and Stage22B is a reproducible demonstration bench rather than real market settlement.

### Startup Protocol For Claude Code / Codex

1. Read `PROGRESS.md` top section.
2. Read `docs/reports_index.md`.
3. For thesis final writing, read `reports/thesis_final_writing_handoff_2026-05-07.md`.
4. Then read `reports/project_status_2026-05-05.md` and `reports/module_progress_assessment_2026-05-05.md`.
5. For dispatch-side DL, read `data/processed/pvdaq_nsrdb_2020_2022/stage20b_two_stage_policy_report.md` before the older Stage20 report.
6. Treat all older `Next Task` entries below as historical handover records, not current execution instructions.
...
## Handover Snapshot - 2026-05-05 Stage21A Rawhide Weather and Market Feasibility

- Completed: 完成 `Stage21A Rawhide 天气与市场场景数据可行性验证`。验证 Rawhide 坐标 `40.8606, -105.0189` 可通过 Open-Meteo Forecast API 返回 48h 小时级天气字段，字段覆盖 GHI/DNI/DHI、温湿度、气压、降水、风速/风向/阵风和分层云量，可映射到 Stage7/Stage21 标准天气 schema。
- Changed Files: `reports/stage21a_rawhide_weather_market_feasibility_report.md`, `configs/market_price_scenarios.stage21a.json`, `docs/reports_index.md`, `PROGRESS.md`.
- Key Result: 推荐推进 `Stage21 Rawhide weather-driven price-scenario dispatch`，但保留 Stage18 为 PVDAQ 容量缩放基准。Stage21A 结论是天气驱动与价格场景可配置方案可实现；它不证明 Rawhide 真实历史收益，也不替换 Stage20B 神经调度结论。
- Market Result: 复用并更新 Stage15A 市场边界。SPP WEIS RTBM LMP 可作为 `2023-04-01` 至 `2026-03-31` 扩展验证候选；`2026-04-01` 后 Platte River 已进入 SPP RTO，应优先查 SPP Integrated Marketplace RTBM/DA LMP。`2020-2022` 主实验期仍不能用 SPP/WEIS/RTO 价格回填为 Rawhide 真实结算。
- Price Schema: 新增 `configs/market_price_scenarios.stage21a.json`，定义 `flat_proxy_30`, `tou_peak_valley`, `solar_duck_curve`, `high_volatility_stress`, `opsd_profile_reference` 五类场景，并强制保留 `is_real_settlement_price`，避免把典型曲线误写为真实市场价格。
- Verification: Open-Meteo Rawhide Forecast API 实际请求成功，返回网格点 `40.85318, -105.03476`、海拔 `1727.0 m`、48 行小时级字段；`python -m json.tool configs\market_price_scenarios.stage21a.json` 通过；`python -m compileall -q src` 通过。
- Next Task: 若继续工程实现，进入 `Stage21`：接入 Open-Meteo 标准天气表，按 Rawhide 容量用透明物理近似估算 PV 出力，消费 `market_price_scenarios.stage21a.json` 生成小时级 `price_eur_mwh`，复用 Stage12 rolling 调度并输出场景级报告。
- Startup Context: 先读 `reports/stage21a_rawhide_weather_market_feasibility_report.md`。Stage21A 是数据门禁，不是调度结果；后续不得把 synthetic price scenario 或 SPP 代理节点写成 Rawhide 真实结算价格。
- Pitfall: Stage21A 的最大风险仍是电价节点映射和真实出力缺失。天气 API 可用只解决 PV 出力估算输入，不解决 Rawhide 实测发电和真实结算收益。
## Handover Snapshot - 2026-05-06 Stage21 Weather-Price Dispatch and Engineering Guardrails

- Completed: 按“先补工程护栏，再推进 Stage21，最后轻量生产化加固”的路线完成实现。新增 Stage21 Rawhide 天气驱动与电价场景调度模块，输入 Open-Meteo 兼容天气 CSV、本地 Rawhide 配置和 `market_price_scenarios.stage21a.json`，用 `capacity_kw * ghi_wm2 / 1000 * performance_ratio` 估算 PV，并复用 Stage12 rolling optimization 输出五类价格场景的调度结果。
- Changed Files: `.gitignore`, `.env.example`, `.github/workflows/ci.yml`, `pytest.ini`, `configs/market_price_scenarios.stage21a.json`, `src/new_energy_sys/stage21_rawhide_weather_dispatch.py`, `src/new_energy_sys/cli/run_stage21_rawhide_weather_dispatch.py`, `tests/test_storage_dispatch_core.py`, `tests/test_stage21_market_scenarios.py`, `tests/fixtures/stage21_rawhide_weather_sample.csv`, `backend/app/data_loader.py`, `backend/app/main.py`, `backend/app/tasks.py`, `frontend/src/services/dispatchService.js`, `frontend/src/charts/dispatchCharts.js`, `frontend/src/views/DispatchSimulation.vue`, `frontend/tests/e2e/frontend.spec.js`, `scripts/api_smoke_frontend_contract.py`, `pyproject.toml`, `docs/reports_index.md`, `PROGRESS.md`.
- Generated Artifacts: `data/processed/pvdaq_nsrdb_2020_2022/stage21_rawhide_weather_predictions.csv`, `stage21_rawhide_price_scenarios.csv`, `stage21_rawhide_dispatch_results.csv`, `stage21_rawhide_dispatch_metrics.csv`, `stage21_rawhide_weather_price_dispatch_report.json`, `stage21_rawhide_weather_price_dispatch_report.md`.
- Key Result: Stage21 生成 `flat_proxy_30`, `tou_peak_valley`, `solar_duck_curve`, `high_volatility_stress`, `opsd_profile_reference` 五类价格场景。质量门禁全部通过：天气预测非空、PV 估算未越界、明确 `is_measured_rawhide_generation=false`、价格场景生成成功、SOC/功率/禁止同时充放电/能量守恒门禁全通过。示例结果中 rolling optimization 增量收益分别为 `0.0000`, `104.8543`, `217.2114`, `601.6364`, `35.4851 EUR`。
- API/Frontend Result: 新增只读接口 `/api/stage21/report`, `/api/stage21/weather-predictions`, `/api/stage21/price-scenarios`, `/api/stage21/dispatch-results`, `/api/stage21/dispatch-metrics`；缺少 Stage21 产物时返回明确 404，不静默返回空数组。前端调度页新增 `Stage21 场景` 页签、价格场景选择器、天气 PV/价格曲线、SOC/充放电功率曲线、场景收益对比和固定边界提示。
- Engineering Guardrails: `.gitignore` 增补 `.local_secrets/` 与 `*accessKeys*.csv`；`Yojimbo_accessKeys.csv` 已移入 `.local_secrets/`；`new-energy-hrrr-usw1.pem` 因 Windows 权限拒绝未能移动，但 `*.pem` 已被忽略。新增最小 CI，仅运行核心 Python 单测与前端 `npm run check`；API 与任务层接入 `logging`；新增 `.env.example`。
- Verification: `python -m pytest tests\test_storage_dispatch_core.py tests\test_stage21_market_scenarios.py -q` 通过，`9 passed`；`python -m json.tool configs\market_price_scenarios.stage21a.json` 通过；`$env:PYTHONPATH='src'; python -m new_energy_sys.cli.run_stage21_rawhide_weather_dispatch ...` 通过并生成 Stage21 产物；`python scripts\api_smoke_frontend_contract.py` 通过并覆盖 Stage21 接口；`cd frontend; npm run check` 通过；`cd frontend; npm run build` 通过；`python -m compileall -q src backend` 通过。
- E2E Status: 已把 Stage21 页签和边界提示加入 Playwright E2E 用例，但当前 Codex sandbox 下 `npm run test:e2e -- --project=system-chromium` 因 `spawn EPERM` 失败；两次请求提升权限均超时，未完成浏览器级验收。
- Next Task: 若继续工程验收，优先在用户本机普通 PowerShell/IDE Terminal 中运行 `cd frontend; npm run test:e2e -- --project=system-chromium`，确认 `#/dispatch` 的 Stage21 页签、场景选择器和边界提示；随后可把 Stage21 结果写入论文“扩展实验/边界分析”章节。
- Pitfall: Stage21 是“Rawhide 坐标天气驱动 + 可配置价格场景”的仿真，不是 Rawhide 实测发电回放；`opsd_profile_reference` 仍是项目代理价格，synthetic scenarios 不是真实结算价格，报告和论文中不得写成 Rawhide 真实市场收益。
