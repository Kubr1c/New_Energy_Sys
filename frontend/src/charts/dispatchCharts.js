...
function money(value) {
  const n = Number(value)
  return Number.isFinite(n) ? `${n.toFixed(2)} 欧元` : 'N/A'
}
...
export function buildScoreBarOption(scorecard) {
  ...
  return {
    ...
    legend: { data: ['经济性', '约束满足', '运行风险'], top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    grid: { left: 150, right: 30, top: 44, bottom: 24 },
    xAxis: { type: 'value', max: 100, axisLabel: { color: 'rgba(255,255,255,0.74)' } },
    yAxis: {
      type: 'category',
      data: scorecard.map(item => scenarioLabel(item.scenario_id)),
      axisLabel: { fontSize: 11, color: 'rgba(255,255,255,0.82)' },
    },
    series: [
      ...
      { name: '运行风险', type: 'bar', stack: 'score', data: scorecard.map(item => (Number(item.risk_score) / 3).toFixed(1)), itemStyle: { color: chartColors.orange } },
    ],
  }
}
...
export function buildGovernanceRadarOption(scorecard) {
  ...
  return {
    legend: { data: rows.map(item => scenarioLabel(item.scenario_id)), top: 0, textStyle: { fontSize: 10, color: 'rgba(255,255,255,0.82)' } },
    radar: {
      indicator: [
        { name: '经济性', max: 100 },
        { name: '约束满足', max: 100 },
  ...
  return {
    tooltip: {
      ...
      formatter(params) {
        ...
        if (!row) return ''
        return `${scenarioLabel(row.scenario)}<br/>相对无储能基准的增量收益：${money(row.incremental_revenue_eur || 0)}`
      },
    },
    ...
    xAxis: {
      ...
      axisLabel: { interval: 0, rotate: 16, fontSize: 11, color: 'rgba(255,255,255,0.82)' },
    },
    yAxis: {
      type: 'value',
      name: '仿真收益（欧元）',
      axisLabel: { formatter: value => `${value} 欧元`, color: 'rgba(255,255,255,0.74)' },
    ...
    series: [
      {
        ...
        label: { show: true, position: 'top', formatter: params => `${Number(params.value).toFixed(0)} 欧元`, fontSize: 10, color: 'rgba(255,255,255,0.86)' },
      },
    ],
  }
}
...
export function buildStage21WeatherPriceOption(weatherRows, priceRows, scenarioId) {
  ...
  return {
    tooltip: { trigger: 'axis' },
    legend: { data: ['天气估算功率', '太阳辐照强度', '电价'], top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    grid: { left: 72, right: 70, top: 44, bottom: 56 },
    xAxis: { type: 'category', data: rows.map(item => formatHourLabel(item.time)), axisLabel: { interval: 3, rotate: 18, fontSize: 10, color: 'rgba(255,255,255,0.74)' } },
    yAxis: [
      { type: 'value', name: 'kW / Wm2', axisLabel: { formatter: value => Number(value).toFixed(0), color: 'rgba(255,255,255,0.74)' } },
      { type: 'value', name: '代理电价（欧元/MWh）', axisLabel: { formatter: value => `${Number(value).toFixed(0)} 欧元`, color: 'rgba(255,255,255,0.74)' } },
    ],
    series: [
      { name: '天气估算功率', type: 'line', smooth: true, symbol: 'none', data: rows.map(item => item.pv.toFixed(2)), itemStyle: { color: chartColors.cyan } },
      { name: '太阳辐照强度', type: 'line', smooth: true, symbol: 'none', data: rows.map(item => item.ghi.toFixed(2)), itemStyle: { color: chartColors.orange } },
      { name: '电价', type: 'line', yAxisIndex: 1, smooth: true, symbol: 'none', data: rows.map(item => item.price.toFixed(2)), itemStyle: { color: chartColors.green } },
    ],
  }
}
...
export function buildStage21SocPowerOption(results, scenarioId, dispatchMode = 'smooth') {
  ...
  return {
    tooltip: { trigger: 'axis' },
    legend: { data: ['SOC', '充电功率', '放电功率'], top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    title: { text: modeLabel(dispatchMode), left: 8, top: 4, textStyle: { color: 'rgba(255,255,255,0.78)', fontSize: 12, fontWeight: 500 } },
    grid: { left: 70, right: 62, top: 44, bottom: 56 },
    xAxis: { type: 'category', data: rows.map(item => formatHourLabel(item.time)), axisLabel: { interval: 3, rotate: 18, fontSize: 10, color: 'rgba(255,255,255,0.74)' } },
    yAxis: [
      { type: 'value', name: 'kW', axisLabel: { formatter: value => Number(value).toFixed(0), color: 'rgba(255,255,255,0.74)' } },
      { type: 'value', name: 'SOC %', min: 0, max: 100, axisLabel: { formatter: value => `${Number(value).toFixed(0)}%`, color: 'rgba(255,255,255,0.74)' } },
    ],
    series: [
      { name: '充电功率', type: 'bar', stack: 'power', data: rows.map(item => item.charge.toFixed(2)), itemStyle: { color: chartColors.cyan } },
      { name: '放电功率', type: 'bar', stack: 'power', data: rows.map(item => item.discharge.toFixed(2)), itemStyle: { color: chartColors.orange } },
      { name: 'SOC', type: 'line', yAxisIndex: 1, smooth: true, symbol: 'none', data: rows.map(item => item.soc.toFixed(2)), itemStyle: { color: chartColors.green } },
    ],
  }
}
...
export function buildStage21ScenarioRevenueOption(metrics) {
  ...
  return {
    ...
    legend: { data: visibleModes.map(mode => (mode === 'legacy' ? '滚动优化调度方案' : modeLabel(mode))), top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    grid: { left: 86, right: 26, top: 44, bottom: 78 },
    xAxis: {
      type: 'category',
      data: scenarios.map(([, label]) => label),
      axisLabel: { interval: 0, rotate: 22, fontSize: 10, color: 'rgba(255,255,255,0.78)' },
    },
    yAxis: { type: 'value', name: '仿真收益（欧元）', axisLabel: { formatter: value => `${Number(value).toFixed(0)} 欧元`, color: 'rgba(255,255,255,0.74)' } },
    series: visibleModes.map(mode => {
      ...
      return {
        ...
        type: 'bar',
        data: modeRows.map(item => toNumber(item.incremental_revenue_eur).toFixed(2)),
...
function selectedMarkLine(rows, selectedIndex) {
  ...
  return {
    silent: true,
    symbol: 'none',
    lineStyle: { color: 'rgba(255,255,255,0.36)', type: 'dashed' },
    label: { show: false },
...
function baseExperimentAxis(rows, selectedIndex, right = 36) {
  return {
    ...
    xAxis: {
      ...
      axisLabel: { interval: rows.length > 48 ? 7 : rows.length > 24 ? 5 : 3, rotate: 18, fontSize: 10, color: 'rgba(255,255,255,0.74)' },
    },
...
export function buildExperimentWeatherTrendOption(rows, selectedIndex = 0) {
  ...
  return {
    ...
    legend: { data: ['辐照度', '云量', '温度'], top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    grid: base.grid,
    xAxis: base.xAxis,
    yAxis: [
      { type: 'value', name: 'W/m2', axisLabel: { color: 'rgba(255,255,255,0.74)' } },
      { type: 'value', name: '°C / %', axisLabel: { color: 'rgba(255,255,255,0.74)' } },
    ],
    series: [
...
export function buildExperimentPvForecastOption(rows, selectedIndex = 0) {
  ...
  return {
    ...
    legend: { data: ['光伏预测功率', '辐照度'], top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    grid: { ...base.grid, bottom: 72 },
    xAxis: base.xAxis,
    yAxis: [
      { type: 'value', name: 'kW', axisLabel: { color: 'rgba(255,255,255,0.74)' } },
      { type: 'value', name: 'W/m2', axisLabel: { color: 'rgba(255,255,255,0.74)' } },
    ],
    series: [
...
export function buildExperimentPowerDispatchOption(rows, selectedIndex = 0) {
  ...
  return {
    tooltip: base.tooltip,
    legend: { data: ['光伏预测功率', '储能充电功率', '储能放电功率', '并网功率'], top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    grid: base.grid,
    xAxis: base.xAxis,
    yAxis: { type: 'value', name: 'kW', axisLabel: { color: 'rgba(255,255,255,0.74)' } },
    series: [
      { name: '光伏预测功率', type: 'line', smooth: true, symbol: 'none', data: rows.map(item => toNumber(item.pvKw).toFixed(2)), itemStyle: { color: chartColors.cyan }, markLine: base.selectedLine },
      { name: '储能充电功率', type: 'bar', stack: 'storage', data: rows.map(item => (-toNumber(item.chargeKw)).toFixed(2)), itemStyle: { color: chartColors.blue } },
      { name: '储能放电功率', type: 'bar', stack: 'storage', data: rows.map(item => toNumber(item.dischargeKw).toFixed(2)), itemStyle: { color: chartColors.orange } },
      { name: '并网功率', type: 'line', smooth: true, symbol: 'none', data: rows.map(item => toNumber(item.gridKw).toFixed(2)), itemStyle: { color: chartColors.green } },
    ],
  }
}
...
export function buildExperimentSocOption(rows, selectedIndex = 0) {
  ...
  return {
    tooltip: base.tooltip,
    legend: { data: ['SOC', 'SOC下限', 'SOC上限'], top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    grid: base.grid,
    xAxis: base.xAxis,
    yAxis: { type: 'value', name: 'SOC %', min: 0, max: 100, axisLabel: { formatter: value => `${Number(value).toFixed(0)}%`, color: 'rgba(255,255,255,0.74)' } },
    series: [
...
export function buildExperimentPriceRevenueOption(rows, selectedIndex = 0) {
  ...
  return {
    tooltip: base.tooltip,
    legend: { data: ['电价', '小时收益', '增量收益'], top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    grid: base.grid,
    xAxis: base.xAxis,
    yAxis: [
      { type: 'value', name: '仿真收益（欧元）', axisLabel: { color: 'rgba(255,255,255,0.74)' } },
      { type: 'value', name: '代理电价（欧元/MWh）', axisLabel: { color: 'rgba(255,255,255,0.74)' } },
    ],
    series: [
      { name: '小时收益', type: 'bar', data: rows.map(item => toNumber(item.revenueEur).toFixed(2)), itemStyle: { color: chartColors.green }, markLine: base.selectedLine },
      { name: '增量收益', type: 'bar', data: rows.map(item => toNumber(item.incrementalRevenueEur).toFixed(2)), itemStyle: { color: params => (Number(params.value) >= 0 ? chartColors.cyan : chartColors.red) } },
      { name: '电价', type: 'line', yAxisIndex: 1, smooth: true, symbol: 'none', data: rows.map(item => toNumber(item.priceEurMwh).toFixed(2)), itemStyle: { color: chartColors.orange } },
    ],
  }
}
...
export function buildExperimentComparisonOption(rows) {
  ...
  return {
    tooltip: { trigger: 'axis', valueFormatter: value => money(value) },
    legend: { data: ['增量收益'], top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    grid: { left: 82, right: 30, top: 44, bottom: 74 },
    xAxis: {
      type: 'category',
      data: rows.map(item => item.label),
      axisLabel: { interval: 0, rotate: 14, fontSize: 10, color: 'rgba(255,255,255,0.78)' },
    },
    yAxis: { type: 'value', name: '仿真增量收益（欧元）', axisLabel: { color: 'rgba(255,255,255,0.74)' } },
    series: [
      { name: '增量收益', type: 'bar', data: rows.map(item => toNumber(item.incrementalRevenueEur).toFixed(2)), itemStyle: { color: params => (Number(params.value) >= 0 ? chartColors.cyan : chartColors.red) } },
    ],
  }
...
export function buildExperimentSensitivityOption(rows) {
  ...
  return {
    tooltip: { trigger: 'axis', valueFormatter: value => money(value) },
    legend: { data: ['容量敏感性', '功率敏感性'], top: 0, textStyle: { color: 'rgba(255,255,255,0.82)' } },
    ...
    yAxis: { type: 'value', name: '仿真收益（欧元）', axisLabel: { color: 'rgba(255,255,255,0.74)' } },
    series: [
      { name: '容量敏感性', type: 'line', smooth: true, data: rows.map(item => toNumber(item.capacityRevenueEur).toFixed(2)), itemStyle: { color: chartColors.cyan } },
      { name: '功率敏感性', type: 'line', smooth: true, data: rows.map(item => toNumber(item.powerRevenueEur).toFixed(2)), itemStyle: { color: chartColors.orange } },
    ],
  }
}
