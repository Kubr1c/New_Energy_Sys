  ...
  <div class="report-viewer">
    ...
    <div class="rv-layout">
      <aside class="glass-card stage-list">
        ...
        >
          <span class="stage-badge">报告</span>
          <span class="stage-label">{{ stageLabel(stage.stage_id, stage.name) }}</span>
      ...
      <section class="glass-card report-content">
        ...
        <PageState v-else-if="error && !mdContent" type="error" title="报告加载失败" :message="error.message" retryable @retry="selected && selectStage(selected)" />
        <div v-else-if="mdContent" class="markdown-body" v-html="renderedMd"></div>
        <PageState v-else type="empty" title="请选择项目报告" :message="`左侧共有 ${stages.length} 份可用报告，进入页面会默认打开第一份报告。`" />
      </section>
    </div>
  </div>
</template>
...
import { reportLabel } from '../utils/displayLabels'
...
const reportInsight = computed(() => ({
  title: stages.value.length
    ? `当前共有 ${stages.value.length} 份报告，默认打开 ${stageLabel(currentStage.value.stage_id, currentStage.value.name || stages.value[0]?.name)}。`
    : '当前没有可用报告，请先生成项目报告产物。',
  tone: stages.value.length ? 'positive' : 'warning',
  items: [
    `当前选中报告：${stageLabel(currentStage.value.stage_id, currentStage.value.name) || '未选择'}。`,
    `当前报告格式：${currentStage.value.has_md ? 'Markdown' : currentStage.value.stage_id ? 'JSON' : '数据缺失'}。`,
    error.value ? `最近一次加载提示：${error.value.message}` : '报告正文保留历史原貌，页面仅优化报告入口名称。',
  ],
}))

function stageLabel(stageId, name) {
  return reportLabel(stageId, name)
}
...
onMounted(async () => {
  try {
    ...
    if (stages.value.length) await selectStage(stages.value[0].stage_id)
  } catch (e) {
    error.value = e.normalized || normalizeApiError(e)
  }
})
...
.rv-layout { display: grid; grid-template-columns: 300px minmax(0, 1fr); gap: var(--space-lg); min-height: 0; flex: 1; }
.stage-list { min-width: 0; overflow-y: auto; padding: var(--space-lg); }
.stage-header { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; margin-bottom: var(--space-md); }
...
.stage-btn { width: 100%; display: grid; grid-template-columns: 42px minmax(0, 1fr) 42px; align-items: center; gap: 8px; border: 0; border-radius: var(--radius-sm); background: transparent; color: var(--text-secondary); cursor: pointer; margin-bottom: 5px; padding: 8px 10px; text-align: left; }
...
.stage-badge { color: var(--accent-cyan); font-size: 11px; font-weight: 800; }
.stage-label { font-size: 12px; line-height: 1.35; min-width: 0; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }
...
.report-loading { min-height: 260px; display: flex; align-items: center; justify-content: center; gap: 12px; color: var(--text-secondary); }
