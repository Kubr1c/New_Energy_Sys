...
test.describe('NES frontend production contract', () => {
  test('logs in without exposing demo credentials and opens all core routes', async ({ page }) => {
    ...
    await login(page)
    await expectRouteMarker(page, '/#/', '储能调度多收益情景量化展示系统')
    ...
    await expectRouteMarker(page, '/#/dispatch', '收益情景')
    await page.getByRole('tab', { name: /天气驱动实验台/ }).click()
    await expect(page.locator('body')).toContainText('参数可调的储能优化调度入口')
    await expect(page.locator('body')).toContainText('仿真边界')
    await expect(page.getByRole('button', { name: /运行调度/ })).toBeVisible()
    await expectRouteMarker(page, '/#/governance', '储能配置敏感性明细')
    await expectRouteMarker(page, '/#/data', '预测特征贡献 Top20')
    await page.locator('.explorer-tabs label').filter({ hasText: '调度收益贡献' }).click({ force: true })
    await expect(page.locator('body')).toContainText('调度收益影响程度')
    await expect(page.locator('body')).not.toContainText('核心结论')
  })
  ...
  test('shows a visible permission error when a guest submits a task', async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText('预测特征贡献 Top20', { timeout: 30_000 })

    await page.getByRole('button', { name: /训练主预测模型/ }).first().click()
    await expect(page.locator('.task-feedback.failed')).toBeVisible()
    await expect(page.locator('.task-feedback.failed')).toContainText('任务创建失败')
  })
})
